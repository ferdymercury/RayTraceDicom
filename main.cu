...
int main()
{
    ...
    imIdxToWorld = Float3AffineTransform (Matrix3x3(1.0f, 1.0f, 1.0f), make_float3(-128.0f, -128.0f, -256.0f+150.0f));
    ...
    //  Matrix3x3 imDir(make_float3(itkImDir[0]), make_float3(itkImDir[1]), make_float3(itkImDir[2]));
    //  Matrix3x3 imSpacing(make_float3(itkImSpacing.GetDataPointer()));
    //  float3 imOrigin = make_float3(itkImOrigin.GetDataPointer());
    //  imIdxToWorld = Float3AffineTransform(imDir*imSpacing, imOrigin);
    ...
    //  // Testing, change to real vector argument
